chrome.storage.local.get(['connectionStatus', 'tailscaleIP', 'ipHistory', 'netflixTabId'], (result) => {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs.length > 0) {
      ...
      chrome.storage.local.get(['ipHistory'], (result) => {
        ...
        // Store the Netflix tab ID so disconnect targets the right tab
        chrome.storage.local.set({
          ...
          chrome.tabs.sendMessage(tabId, { action: 'start', ip: ip });
          window.close();
        });
      });
    }
...
document.getElementById('disconnectBtn').addEventListener('click', () => {
  ...
  // Use the stored Netflix tab ID instead of the currently active tab
  chrome.storage.local.get(['netflixTabId'], (result) => {
    const tabId = result.netflixTabId;
    if (tabId) {
      chrome.tabs.sendMessage(tabId, { action: 'disconnect' }).catch(() => {});
    }
  });
